...
describe("enqueueGeneration", () => {
  it("ejecuta la generación de forma asíncrona (fire-and-forget) vía runGenerationInline", () => {
    vi.clearAllMocks();
    enqueueGeneration("ord_1");
    expect(runGenerationInline).toHaveBeenCalledWith("ord_1");
  });

  it("no lanza si la generación inline falla", async () => {
    vi.clearAllMocks();
    vi.mocked(runGenerationInline).mockRejectedValueOnce(new Error("boom"));

    expect(() => enqueueGeneration("ord_1")).not.toThrow();
    await vi.waitFor(() => expect(runGenerationInline).toHaveBeenCalledWith("ord_1"));
  });
});
